...
set -e
sudo debootstrap --arch amd64 --cache-dir `pwd`/debootstrap-cache --include grub2,grub-efi-amd64-bin,linux-image-amd64,btrfs-progs,sudo,lvm2,cryptsetup-bin,cryptsetup-initramfs,cryptsetup-run buster mnt/ https://deb.debian.org/debian/ 
...
grub-install --target=x86_64-efi --efi-directory=/boot/efi --root-directory=/ \"$BLOCK_DEVICE\" --removable && \
update-grub && \
